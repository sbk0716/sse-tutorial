...
import { logSSEConnection, logSSEDisconnection, logInfo } from '../utils/logger';
...
export const connectSSE = (req: Request, res: Response): void => {
  ...
  if (clients.length >= MAX_CLIENTS) {
    logInfo(`SSE接続拒否 - 最大接続数(${MAX_CLIENTS})に到達 - クライアント: ${clientIP}`);
  ...
  if (lastEventId) {
    logInfo(`Last-Event-ID検出 - クライアントID: ${clientId} - ID: ${lastEventId}`);
  ...
  const timeout = setTimeout(
    () => {
      ...
      if (index !== -1) {
        logInfo(`SSE接続タイムアウト - クライアントID: ${clientId} - 接続時間: 30分`);
...
export const connectSecureSSE = (req: Request, res: Response): void => {
  ...
  const lastEventId = req.headers['last-event-id'];
  if (lastEventId) {
    logInfo(`認証済みSSE - Last-Event-ID検出 - クライアントID: ${clientId} - ID: ${lastEventId}`);
    sendMissedEvents(res, parseInt(lastEventId as string));
  }
